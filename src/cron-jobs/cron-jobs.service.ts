...
export class CronJobsService implements OnModuleInit {
    ...
    async addCronJob(name: string, expireDate: Date, targetId: number, jobType: cronJobName): Promise<void> {
        // создаём CronJob
    ...
    async createCronJob(jobType: cronJobName, targetId: number): PromiseOptional<void> {
